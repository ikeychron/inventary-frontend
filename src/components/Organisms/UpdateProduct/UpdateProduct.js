...
const UpdateProduct = ({ open, handleClose, productId, getProducts }) => {
  ...
  const getProduct = () => {
    Axios.post("http://localhost:4000/product", { id: productId })
      .then(({ data }) => {
      ...
      .catch(({ response }) => {
        if (response.data.error) {
          ...
          setError(typeof error.msg === "object" ? error[0].msg : error.msg);
        }
      });
  };
  ...
  return (
    <Formik
      ...
      onSubmit={(values, { resetForm }) => {
        Axios.put("http://localhost:4000/product", { ...values, id: productId })
          .then(({ data }) => {
